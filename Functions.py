...
def KNN(dist_list, label_list):
    ...
    # If multiple labels have the same occurrence count, select the label with the minimum sum of distances
...
def load_similarity_statistics(dataset_name: str, w: int):
    ...
    if os.path.exists(csv_path):
        try:
            ...
            print(f"Error reading similarity statistics file: {e}")

    print(f"Statistics for dataset {dataset_name} not found, generating automatically...")
    try:
        data_path = f'C:\\Users\\luoyu\\Desktop\\Univariate_arff\\{dataset_name}\\{dataset_name}_TRAIN.txt'
        if not os.path.exists(data_path):
            print(f"Error: Training file for dataset {dataset_name} does not exist: {data_path}")
            return None

        results = analyze_similarity_thresholds(dataset_name, w)
        save_similarity_threshold_results(results)
        print(f"Successfully generated statistics for dataset {dataset_name}")
        return results['same_class']['median']
    except Exception as e:
        print(f"Error generating statistics for dataset {dataset_name}: {e}")
        return None


def KNN_DTW_original(dataset_name: str, w: int, k: int):
    """Plain KNN-DTW without pruning. Returns (accuracy, total_time)."""
    print(f"Start original KNN-DTW classification: {dataset_name}, w={w}, k={k}")
    train_data = txt_loader(f'C:\\Users\\luoyu\\Desktop\\Univariate_arff\\{dataset_name}\\{dataset_name}_TRAIN.txt')
    test_data = txt_loader(f'C:\\Users\\luoyu\\Desktop\\Univariate_arff\\{dataset_name}\\{dataset_name}_TEST.txt')

    print(f"Training templates: {len(train_data)}")
...
def KNN_DTW_with_pruning(dataset_name: str, w: int, k: int, same_class_median: float,
    ...
    print(f"Start KNN-DTW classification (with pruning): {dataset_name}, w={w}, k={k}")
    print(f"Same class distance median: {same_class_median:.6f}")
    print(f"Pruning coefficient: {pruning_coefficient}")
    print(f"Pruning threshold: {pruning_threshold:.6f}")
    print(f"Early stop threshold: {early_stop_count} distances < {pruning_threshold:.6f}")
